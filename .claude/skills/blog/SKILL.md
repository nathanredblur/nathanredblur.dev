...
4. **`references/image-prompts.md`** — How to write an article-only cover image prompt and generate the cover directly via the Gemini API (the fixed style lives in `references/image-style-system-prompt.md`) — read this before generating any cover image
...
Offer to generate the cover image (or do it automatically if the post has `image: ""`). Follow **Mode: Image Prompt** below. The image is generated directly via the Gemini API and saved into the post's folder as `cover.png`.

---

## Mode: Image Prompt

Write an article-only prompt for the post's cover image, then generate the cover directly via the Gemini API. The prompt **only describes the article and key elements** — the fixed visual style is applied by `references/image-style-system-prompt.md`, so never include style, palette, aspect ratio, or composition direction.
...
### Step 3: Generate and save the cover

Run `scripts/generate-cover-image.sh` with the article-only prompt and an `--output` of `src/content/posts/<slug>/cover.png`, following the command in the reference. It calls the Gemini API and decodes the returned image to that path. Then set the post's `image: "./cover.png"` frontmatter field. If `GEMINI_API_KEY` is not set, hand the author the exact command to run with their own key.
